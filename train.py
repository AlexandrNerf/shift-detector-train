...
def main(cfg: DictConfig):
    ...
    if cfg.logging.push_to_hub:
        login_to_hub()

    if not isinstance(cfg.hyperparams.workers, int):
        cfg.hyperparams.workers = min(16, mp.cpu_count())

    torch.backends.cudnn.benchmark = True

    st = time.time()
    val_set = DetectionDataset(
        img_folder=os.path.join(cfg.val_path, "images"),
    ...
    val_loader = DataLoader(
        ...
        drop_last=False,
        num_workers=cfg.hyperparams.workers,
    ...
    # Load doctr model
    model = detection.__dict__[cfg.arch](
        pretrained=cfg.hyperparams.pretrained,
        assume_straight_pages=not cfg.hyperparams.rotation,
        class_names=val_set.class_names,
    )

    # Resume weights
    if isinstance(cfg.resume, str):
        print(f"Resuming {cfg.resume}")
        checkpoint = torch.load(cfg.resume, map_location="cpu")
        model.load_state_dict(checkpoint)

    # GPU
    if isinstance(cfg.hyperparams.device, int):
        if not torch.cuda.is_available():
            raise AssertionError("PyTorch cannot access your GPU. Please investigate!")
        if cfg.hyperparams.device >= torch.cuda.device_count():
            raise ValueError("Invalid device index")
    # Silent default switch to GPU if available
    elif torch.cuda.is_available():
        cfg.hyperparams.device = 0
    else:
        logging.warning("No accessible GPU, targe device set to CPU.")
    if torch.cuda.is_available():
        torch.cuda.set_device(cfg.hyperparams.device)
        model = model.cuda()
    ...
    train_set = DetectionDataset(
        img_folder=os.path.join(cfg.train_path, "images"),
        label_path=os.path.join(cfg.train_path, "labels.json"),
        img_transforms=img_transforms,
        sample_transforms=sample_transforms,
        use_polygons=cfg.hyperparams.rotation,
    )
    ...
    # Backbone freezing
    if cfg.hyperparams.freeze_backbone:
    ...
    exp_name = f"{cfg.arch}_{current_time}" if cfg.name is None else cfg.name
    ...
    if cfg.logging.wb:
        run = wandb.init(
            ...
            config={
                ...
                "optimizer": "adam",
                "framework": "pytorch",
                "scheduler": cfg.hyperparams.sched,
                "train_hash": train_hash,
                "val_hash": val_hash,
                "pretrained": cfg.hyperparams.pretrained,
                "rotation": cfg.hyperparams.rotation,
    ...
    for epoch in range(cfg.epochs):
        ...
        log_msg = f"Epoch {epoch + 1}/{cfg.epochs} - Validation loss: {val_loss:.6} "
    ...
    if cfg.logging.push_to_hub:
        push_to_hf_hub(model, exp_name, task="detection", run_config=cfg)
